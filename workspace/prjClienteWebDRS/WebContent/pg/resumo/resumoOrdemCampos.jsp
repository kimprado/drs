...
var elementoContainerOrdemCampos = null; 

function getListaObjetosOrdemCamposDrag(){
	var lista = new Object();

	var itensDrag = 0;
	for(var i=0; i<elementoContainerOrdemCampos.childNodes.length; i++){

		if(elementoContainerOrdemCampos.childNodes[i].nodeName=='#text') continue;

		lista[itensDrag] = elementoContainerOrdemCampos.childNodes[i];
		itensDrag++;
...
function getObjetoEnviadoParaOrdenacaoCampos(obj){
...
function atualizarOrdemCamposObjetosDrag(){
	var lista = getListaObjetosOrdemCamposDrag();
	var itensDrag = 1;
	for(var i=0; i<lista.size ;i++){
		objetoEnviado = getObjetoEnviadoParaOrdenacaoCampos(lista[i]);
...
window.onload = function(){
	...
	elementoContainerOrdemCampos = document.getElementById('DragContainer1');
...
<form id="frm_salvarOrdemAtributo" name="frm_salvarOrdemAtributo" action="SalvarOrdemAtributo?cube=<%= request.getParameter("cube") %>" method="post">

<div class="DragContainer" id="DragContainerOrdenarCampos">
...
if ( request.getParameter("cube") != null ){
	...
	if (getresumo != null){
		...
		Atributo[] atributos = getresumo.getResumoOrdemAtributos();
		...
		if(atributos != null){
			for (int j=0; j < atributos.length; j++){
				...
				<!-- 	
				if (claroEscuro == true){
					out.println("<tr bgcolor='#ADD8E6'  onclick='mostrarOpcoesAtributo(" + cubeIndexEntry + "," + atributo.getCubo() + "," + atributo.getTabela() + "," + atributo.getCampo() + ",\"" + atributo.getCampoNome() + "\")'>");
					claroEscuro = false;
				}else{
					out.println("<tr bgcolor='#C0EFFF'  onclick='mostrarOpcoesAtributo(" + cubeIndexEntry + "," + atributo.getCubo() + "," + atributo.getTabela() + "," + atributo.getCampo() + ",\"" + atributo.getCampoNome() + "\")'>");
					claroEscuro = true;
				}
				-->
...
<div style="position:left; text-align: right">
	<input type="button" value="Salvar" onclick="salvarOrdenacaoCampos()">
</div>

<script>

function configurarContainerOrdemCampos(){
	...
	CreateDragContainer(document.getElementById('DragContainerOrdenarCampos'));
	elementoContainerOrdemCampos = document.getElementById('DragContainerOrdenarCampos');
	
	//CreateDragContainer(document.getElementById('div_ordem_campos'));
	//elementoContainerOrdemCampos = document.getElementById('div_ordem_campos');
...
function salvarOrdenacaoCampos()
{
	
	var frm = document.getElementById('frm_salvarOrdemAtributo');
	atualizarOrdemCamposObjetosDrag();
	frm.submit();
}
...
function vars()
{
	...
	//alert(lastTarget);
	alert(top);
	top.location='http://localhost:8080/prjClienteWebDRS/';
