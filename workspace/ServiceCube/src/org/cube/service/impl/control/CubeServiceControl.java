...
public class CubeServiceControl {
	...
	public static int setCubeCollectionMetaData(
		...
		try {
			Class.forName(bancoMetadadosDriver); // Inicializa��o do driver
			// jdbc
	...
	public static CubeMetadataResponse getCubeMetadata(Cubo cb) {
		...
		FatoMetaData ftmd = new FatoMetaData(); // a termina��o 'md' se refere a
		// meta data
		...
		for (int i = 0; i < fato.getIdMaxDimensao() + 1; i++) {
			...
			if (fato.contemDimensao(i)) {
				...
				int idLigacao = 0; // obter qtd de liga��es
	...
	public static ExecuteQueryResponse executeQuery(Connection conn, String sql) {
		...
		try {
			...
			// listasResultados Ir� armazenar as cole��es que representam colunas do resultado
			...
			while (rs.next()) { // Varre o resultado da consulta recuperando os campos
				for (int i = 0; i < colSel; i++) {
					if (rs.getObject(i+1) != null){ // Testa se um Campo n�o � Nullo
			...
			// A QTD de Objetos de uma Cole��o � a mesma de linhas recuperadas na Consulta
			linSel = ((LinkedList<String>)listasResultados[0]).size();
			
			 // Recupera os campos das cole��es e monta o resultado
			ColumnResponse[] colresColl = null;
			if (linSel > 0){
				 //Cada posi��o de 'colresColl' aponta para um Objeto que possui um Array que representa todos os valores de determinada coluna
				colresColl = new ColumnResponse[colSel];
				for(int i = 0; i < colSel; i++){
					// Com ToArray do LinkedList s�o recuperados os objetos que representam uma coluna 
	...
	public static void setCubeMetaDataDesativado(Cubo cube, String fato) {
		...
		try {
			...
			while (rsDim.next()) {
				...
				// System.out.println("Nova dimens�o: "+ dm.Getnome());
